...
// 定義 EPSG:2326 的投影參數
const EPSG2326 = '+proj=tmerc +lat_0=22.31213333333334 +lon_0=114.1785555555556 +k=1 +x_0=836694.05 +y_0=819069.8 +ellps=intl +towgs84=-162.619,-276.959,-161.764,0.067753,-2.24365,-1.15883,-1.09425 +units=m +no_defs'
...
const reverseCampsiteUrlMap = campsiteData.features.reduce((acc, feature) => {
  acc[feature.properties.FACILITY_NAME_EN.replace(/ /g, '')] = feature.properties.FACILITY_NAME_TC
  return acc
}, {})
...
const getCampsiteImage = (campsiteName) => {
  ...
  return imageMap[campsiteName] || '/campsite_gemini.jpeg'
}
...
const fetchWindData = async (url) => {
  try {
    ...
    const directionIndex = headers.findIndex(h => h.includes('十分鐘平均風向（方位點）'))
    const speedIndex = headers.findIndex(h => h.includes('十分鐘平均風速（公里/小時）'))
    const maxGustIndex = headers.findIndex(h => h.includes('十分鐘最高陣風風速（公里/小時）'))
...
const createInfoWindowContent = async (properties, nearestStation) => {
  ...
  const tempInfo = temperature !== null 
    ? `<p><strong>溫度:</strong> ${temperature}°C</p>` 
    : ''
  
  const rainfallInfo = rainfall !== null
    ? `<p><strong>雨量:</strong> ${rainfall}mm</p>`
    : ''

  let windInfo = '<p>無風速資料</p>'
  if (windData) {
    windInfo = `
      <p><strong>最近風速監測站:</strong> ${nearestWindStation.properties.AutomaticWeatherStation_uc}</p>
      <p><strong>十分鐘平均風向:</strong> ${windData.direction || 'N/A'}</p>
      <p><strong>十分鐘平均風速:</strong> ${windData.speed?.toFixed(1) || 'N/A'} km/h</p>
      <p><strong>十分鐘最大陣風:</strong> ${windData.max_gust?.toFixed(1) || 'N/A'} km/h</p>
    `
  }
    ...
    <div class="info-window">
      <div class="campsite-image">
        <img 
          ...
          onerror="this.src='/campsite_gemini.jpeg'"
        />
      </div>
      ...
      <div class="campsite-info">
        ...
        <div class="weather-info">
          <p><strong>最近氣象站:</strong> ${nearestStation.properties.weather_station_tc}</p>
          ${tempInfo}
          ${rainfallInfo}
          ${windInfo}
        </div>
...
const handleSearch = () => {
  ...
  searchResults.value = campsiteData.features.filter(feature => 
...
const handleResultClick = (result) => {
  ...
  const marker = markers.value.find(m => 
    m.getLatLng().lat === position.lat && 
    m.getLatLng().lng === position.lng
  )
  
  if (marker) {
    const nearestStation = findNearestWeatherStation(position.lat, position.lng)
    markerClickHandler(marker, result, nearestStation)
  }

  // 更新 URL
  const urlId = Object.keys(reverseCampsiteUrlMap).find(key => 
    reverseCampsiteUrlMap[key] === result.properties.FACILITY_NAME_TC
  )
  if (urlId) {
    router.push({
      query: { place: urlId }
...
// 標記點擊處理
const markerClickHandler = async (marker, feature, nearestStation, updateUrl = true) => {
  if (updateUrl) {
  ...
  const content = await createInfoWindowContent(feature.properties, nearestStation)
...
const fetchWeatherData = async () => {
  try {
    ...
    return data.temperature.data.map(station => ({
      ...
      rainfall: data.rainfall.data.find(r => r.place === station.place)?.max ?? 0
    }))
  } catch (error) {
...
const mergeWeatherData = (stations, weather) => {
  return stations.features.map(station => {
    ...
    const weatherInfo = weather.find(w => 
      w.station_name_tc === station.properties.weather_station_tc
    )
...
onMounted(async () => {
  try {
    ...
    // 監聽路由變化
    watch(() => route.query.place, (newPlace) => {
      if (newPlace && map.value) {
        const campsite = campsiteData.features.find(feature => 
          feature.properties.FACILITY_NAME_TC === reverseCampsiteUrlMap[newPlace]
        )
